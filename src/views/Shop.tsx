...
function ShopContent({ address }: { address: string }) {
    ...
    const totalPages = Math.max(1, Math.ceil((allTools?.length || 0) / TOOLS_PER_PAGE));
    
    const visibleTools = useMemo(() => {
        if (!allTools) return [];
        const startIndex = (currentPage - 1) * TOOLS_PER_PAGE;
        return allTools.slice(startIndex, startIndex + TOOLS_PER_PAGE);
    }, [allTools, currentPage]);
    ...
    return (
        <Container fluid>
            <Flex direction="column" gap="xl">
                ...
                {(allTools?.length || 0) > TOOLS_PER_PAGE && (
                    <Center>
                        <Pagination
                            total={totalPages}
                            value={currentPage}
