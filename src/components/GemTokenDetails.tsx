...
export function GemTokenDetails({ address, onClose, onTradeConfirmed }: Props) {
  ...
  if (isLoading) return <Box py="xl"><Center><Loader color="blue" /></Center></Box>;
    ...
    <Stack>
      ...
      <Tabs value={activeTab} onChange={setActiveTab} color="blue" variant="pills">
        ...
        <Tabs.Panel value="trade" pt="md">
          <TradePanel address={address} info={info} tokenBalance={tokenBalance} hashBalance={hashBalance} hashAllowance={hashAllowance} memeAllowance={memeAllowance} symbol={symbol} account={account} refetchPending={refetchPending} onTradeConfirmed={onTradeConfirmed} onTradeConfirmedLocal={onTradeConfirmedLocal} />
        ...
        <Tabs.Panel value="mining" pt="md">
          <MiningPanel
            ...
            onClaimMiningConfirmed={onClaimMiningConfirmed}
      ...
      <UpdateMetadataModal opened={editOpened} onClose={closeEdit} tokenAddress={address} initialLinks={links} onUpdated={refetchPending} onMetadataConfirmed={onMetadataConfirmed} />
      <WhitepaperModal opened={wpOpened} onClose={closeWP} tokenName={name} tokenSymbol={symbol} />
    </Stack>
  );
}

function UpdateMetadataModal({ opened, onClose, tokenAddress, initialLinks, onUpdated, onMetadataConfirmed }: any) {
    ...
    return (
        <Modal opened={opened} onClose={onClose} title="Update Token Links" centered size="md" styles={{ content: { backgroundColor: '#1a1b1e', color: 'white' }, header: { backgroundColor: '#1a1b1e', color: 'white' } }}>
            <Stack>
                ...
                <AppTransactionButton style={{ background: 'linear-gradient(45deg, #007bff, #00d2ff)', color: 'white', border: 'none', fontWeight: 700, marginTop: '10px' }} transaction={() => ({ address: contractGemFun.address, abi: contractGemFun.abi, functionName: "setTokenMetadata", args: [tokenAddress, meta] })} onTransactionConfirmed={() => { onMetadataConfirmed?.(meta); onUpdated(); onClose(); }}>
                    Save Changes
                </AppTransactionButton>
            </Stack>
        </Modal>
