...
export function ToolCard({ tool, address, contractTools, contractStaking }: ToolCardProps) {
    ...
    const { data: claimConditions, isLoading: isLoadingClaimConditions, refetch: refetchClaimConditions } = useReadContract(getClaimConditions, {
    ...
    const { data: isApproved, isLoading: isLoadingIsApproved, refetch: refetchIsApproved } = useQuery({
    ...
    const { data: currencyBalance, isLoading: isCurrencyBalanceLoading, refetch: refetchCurrencyBalance } = useReadContract(balanceOfErc20, {
                ...
                <Grid.Col span={{ base: 12, md: 5 }}>
                    <Flex direction="column" mt={{ base: 'md', md: 0 }} gap="xs" justify="center" h="100%">
                        <AppTransactionButton
                            onTransactionConfirmed={refreshToolState}
                            transaction={async () => {
